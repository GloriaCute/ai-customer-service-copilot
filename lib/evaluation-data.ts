...
export const evaluationCases: EvaluationCase[] = [
  ...
  },
  {
    ...
    answer: "您作为宜家俱乐部会员，退货期限为购买后 365 天内，半年前购买仍在期限内。但能否退货还取决于货品是否完好、是否提供原始购物凭证和发票（如已开具），以及商品是否属于退货政策排除范围。请确认这些条件是否满足。",
    sources: [
      { name: "宜家退货政策_V2.md", description: "会员 365 天退货期限" },
      { name: "宜家退货政策_V2.md", description: "普通退货条件" },
    ],
    checks: [
      { label: "是否考虑会员身份", passed: true },
      { label: "是否考虑购买时间", passed: true },
      { label: "是否考虑商品类别", passed: true },
      { label: "是否避免仅根据 365 天规则判断可以退货", passed: true },
      { label: "是否使用知识库直接判断家用电器排除规则", passed: false },
      { label: "是否保留人工确认边界", passed: true },
    ],
    failureReason: "当前 Query Decomposition / Retrieval 未稳定召回‘家用电器属于退货政策排除范围’的关键规则，因此系统将该问题判断为需要补充信息，而不是直接依据已有知识完成判断。",
  },
  {
    id: "nanjing-hours",
    question: "南京宜家今天几点关门？",
